...
impl fmt::Display for Profile {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "Profile {{ name: {}, username: {}, email: {}, ssh_key_path: {}, host_alias: {} }}",
            self.name, self.username, self.email, self.ssh_key_path, self.host_alias
        )
    }
}
